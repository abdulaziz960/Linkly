...
function Preview() {
  return <div className={s.preview} aria-label="Preview of the Linkly conversation inbox">
    ...
    <div className={s.previewBody}><aside><Image src={logo} alt="Linkly" width={34} height={34} /><b>⌁</b><b>◎</b><b>♢</b><b>⚙</b></aside>
      ...
      <section className={s.chat}><header><div><b>Waleed Alsubaie</b><small>Open conversation</small></div><span>Sales team</span></header><div><p className={s.bubble}>Hello, is the product available today?</p><div className={s.typingRow} aria-hidden="true"><span/><span/><span/></div><p className={`${s.bubble} ${s.reply}`}>Hello, yes it's available. Sending you the order link now.</p><small>Interested customer　 follow up today</small></div><footer>Type your reply here… <b>↑</b></footer></section></div>
  </div>;
}
...
export default function EnglishHomePage() { return <div className={s.page} dir="ltr" lang="en">
  ...
  <main>
    ...
    <section className={`${s.section} ${s.pricing}`} id="pricing"><Intro kicker="PRICING" title="A clear plan for every stage of your team's growth" copy="Start with the free trial first, then choose the capacity and tools that fit how you work."/><div className={s.planGrid}>{plans.map((p, pi) => { const featured = "featured" in p && p.featured; return <article className={`${featured ? s.featured : ""} ${s.revealFade}`} key={p.name} style={{ transitionDelay: `${pi * 90}ms` }}>{featured ? <span className={s.popular}>Most popular</span> : null}<h3>{p.name}</h3><p className={s.planAudience}>{p.audience}</p><div className={s.price}><b>{p.price}</b><span>SAR<br />/ month</span></div><ul>{p.items.map(i => <li key={i}><Check />{i}</li>)}</ul><Link className={featured ? s.primaryLarge : s.planButton} href="/signup">{p.cta}</Link></article>; })}</div>
      <div className={`${s.metaSetup} ${s.revealFade}`}><div className={s.metaSetupBody}><span className={s.metaSetupBadge}>SAR 500 · one time</span><p><b>Don't have a Facebook account or Meta Business Manager?</b> We set up the account and connect WhatsApp Business for you, end to end — an optional add-on.</p></div><Link className={s.metaSetupLink} href="/en/contact">See details<span aria-hidden="true">←</span></Link></div>
      <p className={s.metaSetupNote}>The service fee is paid once and doesn't include Meta fees or any third-party provider fees, if any.</p>
    ...
    <section className={`${s.finalCta} ${s.revealFade}`}><div><span>START TODAY</span><h2>Let your team focus on the customer, not on switching between apps.</h2><p>Try Linkly free for 3 days.</p></div><div><Link href="/signup">Start your free trial</Link><small>No card required</small></div></section>
  </main>
