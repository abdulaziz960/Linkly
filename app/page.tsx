...
export default function HomePage(){return <div className={s.page}>
  ...
  <LandingNav />
  <main>
    <section className={s.hero}><span className={s.heroAurora} aria-hidden="true"/><div><span className={s.eyebrow}><i/> منصة واحدة لكل فريقك</span><h1>رد أسرع على عملائك <strong>ولا تضيع ولا محادثة</strong></h1><p>واتساب، إنستغرام، البريد وتيليجرام — فريقك يرد ويوزّع ويتابع من صندوق وارد واحد، مع جاهزية تيك توك بعد اعتماد صلاحية المراسلة.</p><div className={s.actions}><Link className={s.primaryLarge} href="/signup">ابدأ تجربتك مجانًا</Link><a className={s.secondary} href="#product">شاهد كيف يعمل ←</a></div><small className={s.micro}><Check/> 3 أيام مجانًا <Check/> بدون بطاقة دفع <Check/> نجهّز لك القنوات</small><div className={s.heroMetrics}><div><b>4</b><span>قنوات متاحة، وتيك توك قريبًا</span></div><div><b>1</b><span>صندوق وارد لكل الفريق</span></div><div><b>∞</b><span>سياق واضح لكل محادثة</span></div></div></div><Preview/></section>
