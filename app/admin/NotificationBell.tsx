...
function targetHref(item: AdminNotification) {
  if (item.type === "renewal") return "/admin/alerts";
  return `/admin/logs?client=${item.tenantId}`;
}
...
export default function NotificationBell() {
  ...
  return (
    <div className="admin-notif-bell-wrap" ref={wrapRef}>
      <button type="button" className="admin-notif-bell" onClick={handleToggle} aria-label="الإشعارات">
      ...
      {open ? (
        <div className="admin-notif-dropdown">
          <div className="admin-notif-dropdown-head">الإشعارات</div>
          <div className="admin-notif-dropdown-list">
            ...
            {!items.length ? <p className="admin-empty-state">لا توجد إشعارات حاليًا.</p> : null}
          </div>
        </div>
      ) : null}
    </div>
