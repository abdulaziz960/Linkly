...
export async function POST(request: NextRequest) {
  // Same explicit opt-in as /api/billing/checkout's simulated branch - a
  // misconfigured non-production environment must fail closed by default,
  // not just whenever it happens to lack production/live-mode signals.
  if (process.env.NODE_ENV === "production" || process.env.MOYASAR_LIVE_MODE === "true" || process.env.MOYASAR_SECRET_KEY || process.env.ENABLE_TEST_CHECKOUT !== "true") return NextResponse.json({ error: "المحاكاة غير متاحة في وضع الدفع الحقيقي" }, { status: 403 });
